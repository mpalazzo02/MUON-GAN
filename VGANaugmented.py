...
def preprocess_data(X):
    ...
    for index in range(2,4):
        X[:, index] = gaussian_filter(X[:, index], sigma=4)
...
def train_step(images, importance_scores=None):
    ...
    if importance_scores is not None:
        # Convert importance scores to TensorFlow constant
        importance_weights = tf.constant(importance_scores, dtype=tf.float32)
        emphasis_factor = tf.constant(1.5)
        # Amplify noise based on feature importance
        adjusted_noise = noise * (1 + tf.reshape(importance_weights, (1, -1)) * emphasis_factor)
    else:
        # Use unmodified noise if no importance scores are provided
        adjusted_noise = noise
    
    # Generate images using either adjusted or regular noise based on the condition above
...
for epoch in range(int(1E30)):
    ...
    for file in list_of_training_files:
        ...
        for index in range(2,4):
            X_train[:, index] = gaussian_filter(X_train[:, index], sigma=4)
        ...
        for images_for_batch in train_dataset:
             # Optionally, perform XGBoost evaluation less frequently to save computation
    ...
    # Plot full distribution of feature values for real samples
                    plt.hist(X_test[:, i][y_test == 1], bins=30, alpha=0.5, label='Real', density=False, color='blue')

    # Plot full distribution of feature values for generated samples
                    plt.hist(X_test[:, i][y_test == 0], bins=30, alpha=0.5, label='Generated', density=False, color='red')
